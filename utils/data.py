...
from torchvision.transforms import v2, ColorJitter, Resize
...
class PlantDataset(Dataset):
    def __init__(self, train_csv, test_csv, image_dir, img_size= 224,transform=None):
        ...
        if "train" in train_csv:
            self.is_train = True
            classes = "X4_mean,X11_mean,X18_mean,X50_mean,X26_mean,X3112_mean".split(",")
            aux_classes = list(map(lambda x: x.replace("mean","sd"), classes))  
            subset.extend(classes)
            subset.extend(aux_classes)
            self.df.dropna(subset=subset)
            
            self.df = remove_outliers(self.df, classes)
            self.df = remove_outliers(self.df, aux_classes)
            ...
            # ColorJitter(brightness=0.5,contrast=0.5, saturation=0.4),
        ...
        feature_cols = self.test_df.columns[1:].tolist()
